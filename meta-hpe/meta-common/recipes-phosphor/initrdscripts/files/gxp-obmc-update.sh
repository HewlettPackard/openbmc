...
then
	...
	do
		...
		if [ ${f#$image} = "u-boot" ] || [ ${f#$image} = "section" ]
		then
			curhash=$(sha256sum /dev/$m | awk '{print $1}')
			newhash=$(sha256sum $f | awk '{print $1}')
			if [ $curhash = $newhash ]
			then
				echo "Skipping updating the identical ${f} image."
				continue
			fi
		fi
		...
		if [ ${f#$image} = "section" ]
		then
			sp=$(( ("$(devmem 0xc0000214 8)" && 0x01) ))
			if [ $sp = 1 ]
			then
				dd if=$f bs=1k count=1088 | flashcp /dev/$m
				continue
			fi
		fi
